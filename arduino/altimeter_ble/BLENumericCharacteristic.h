...
template<typename T> class BLENumericCharacteristic : public BLECharacteristic
{
  public:
...
template<typename T> BLENumericCharacteristic<T>::BLENumericCharacteristic(const char* uuid, unsigned char properties,
                        int8_t multiplier, int8_t decimalExponent, int8_t binaryExponent)
   : BLECharacteristic(uuid, properties, sizeof(T), false)
   , currentValue(0)
...
template<typename T> int BLENumericCharacteristic<T>::writeValue()
{
...
template<typename T> int BLENumericCharacteristic<T>::writeValue(float value)
{
  setValue(value);
  return writeValue();
}

template<typename T> void BLENumericCharacteristic<T>::setValue(float value)
{
  currentValue = value;
}

template<typename T> float BLENumericCharacteristic<T>::value()
{
  T rawValue;
